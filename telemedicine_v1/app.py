    ...
    try:
        ...
        for element in data.get("elements", []):
            ...
            hospitals.append({
                ...
                "directions_url": f"https://www.google.com/maps/dir/?api=1&origin={lat},{lon}&destination={lat},{lon}&travelmode=driving"
            })
...
def hospitals():
    try:
        ...
        radius = request.args.get("radius", type=int, default=20000)
...
@app.route('/hospitals/search', methods=['POST'])
def search_hospitals():
    try:
        ...
        # Filter hospitals by search term if provided
        if search_term:
            hospitals = [h for h in hospitals if search_term.lower() in h['name'].lower()]
...
# Hospital maps endpoints (already defined above)
