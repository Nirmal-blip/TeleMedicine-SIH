...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const sendOTP = async (email: string, purpose: 'signup' | 'signin') => {
    try {
      const response = await axios.post(`${BACKEND_BASE_URL}/api/auth/send-otp`, {
        email,
        purpose,
      });
  ...
  const registerWithOTP = async (formData: any, otp: string) => {
    try {
      ...
      const response = await axios.post(`${BACKEND_BASE_URL}/api/auth/register-with-otp`, {
        ...formData,
        otp,
      }, { withCredentials: true });
  ...
  const loginWithOTP = async (email: string, password: string, userType: 'patient' | 'doctor', otp: string) => {
    try {
      ...
      const response = await axios.post(`${BACKEND_BASE_URL}/api/auth/login-with-otp`, {
        email,
        password,
        userType,
        otp,
  ...
  const logout = async () => {
    try {
      await axios.get(`${BACKEND_BASE_URL}/api/auth/logout`, { withCredentials: true });
    } catch (error) {
