...
const Checkout: React.FC = () => {
  ...
  const uploadPrescriptions = async (): Promise<string[]> => {
    ...
    for (const file of prescriptionFiles) {
      ...
      try {
        const response = await fetch(`${(import.meta as any).env.VITE_BACKEND_URL}/api/uploads/prescription`, {
          method: 'POST',
          credentials: 'include',
          body: formData,
        });
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const response = await fetch(`${(import.meta as any).env.VITE_BACKEND_URL}/api/orders`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (paymentMethod === 'razorpay') {
          ...
          if (result) {
            ...
            try {
              const invoiceUrl = `${(import.meta as any).env.VITE_BACKEND_URL}/api/orders/${data.order._id}/invoice`;
  ...
  const initiateRazorpayPayment = (order: any, razorpayOrder: any) => {
    const options = {
      ...
      handler: async (response: any) => {
        try {
          const verifyResponse = await fetch(`${(import.meta as any).env.VITE_BACKEND_URL}/api/orders/verify-payment`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (verifyResponse.ok) {
            ...
            if (result) {
              // Open invoice in new tab
              try {
                const invoiceUrl = `${(import.meta as any).env.VITE_BACKEND_URL}/api/orders/${order._id}/invoice`;
                window.open(invoiceUrl, '_blank');
              } catch (error) {
                console.error('Could not open invoice:', error);
              }
